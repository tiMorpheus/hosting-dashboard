...
add_hook('OrderProductPricingOverride', 0, function ($vars) {
    ...
    logActivity('before trialPricingOverride');
    ...
    logActivity('after TrialPricingOverride');
    ...
    try {
        ...
        if($vars['key'] == 0) {
            ...
            logActivity(' clearing trial products array and set products num is 0');
        ...
        if($vars['proddata']['qty'] > 0) {
            ...
            $options = Capsule::table('tblcustomfields')->where('relid', $vars['pid'])
                ...
                ->where('fieldoptions', '0')
                ->first();
            logActivity('check if it is trial...');

            if(!empty($options)) {
                logActivity('it is trial');

                // get prices for this product
                $price = Capsule::table('tblpricing')->where('relid', $vars['pid'])->where('type', 'product')->first();

                logActivity('1. trialPurchageAllowed '.Util::trialPurchaseAllowed($clientid, $vars['pid']));
                logActivity('2. overridePriceIFPPBa not set '.$overridePriceIfPPBANotSet);

                logActivity('3.Util::getTrialsNotAllowedReasons()[$vars["pid"]] '.Util::getTrialsNotAllowedReasons()[$vars['pid']]);
                logActivity('4.Util::PPBA_NOT_SET '.Util::getTrialsNotAllowedReasons()[$vars['pid']]);
                ...
                if(Util::trialPurchaseAllowed($clientid, $vars['pid']) || ($overridePriceIfPPBANotSet && Util::getTrialsNotAllowedReasons()[$vars['pid']] === Util::PPBA_NOT_SET)) {
                    ...
                    logActivity('trial is not allowed');
                    logActivity('is pending trial? .:' .TrialsIntegration::getInstance()->isPendingTrial() );
                    run_hook('TrialPricingOverrode', [
                        'userid' => $_SESSION['uid'],
                        'pid' => $vars['pid'],
                        'trial' => false,
...
add_hook('InvoiceCreated', 3, function($vars) {
    ...
    foreach ($invoiceitems as $item) {
        ...
        try {
            ...
            logActivity('TRIAL MODULE ERROR -> ' . $e->getMessage() . "\nStacktrace:\n" . $e->getTraceAsString());
        }
